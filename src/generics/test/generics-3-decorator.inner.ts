...
@CastAssign("instance")
@Fqn(...LY_TESTING_FQN)
export class MyKey1 implements CastLike<string> {
...
@CastAssign("instance")
@Fqn(...LY_TESTING_FQN)
export class MyValue1 implements CastLike<string> {
    cast(value: unknown|string): string {
        return leyyo.primitive.string(value);
    }
}
...
@Nickname('nicknameObject1')
@Fqn(...LY_TESTING_FQN)
export class MyObject1<K extends RecKey, V> implements GenericsLike<Record<K, V>> {
    gen(identifier: GenericsAnyIdentifier, value: unknown | Record<K, V>, opt?: GenericsOption): Record<K, V> {
        ...
        if (tree.children.length < 1) {
            tree.children[0] = {base: 'MyKey1'};
            tree.children[1] = {base: 'MyValue1'};
